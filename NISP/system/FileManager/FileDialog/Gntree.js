...
/*		//删除节点
		...
		Gntree.prototype.transformTozTreeNodes = function(path){  
			...
			for(var i=0; i < fileArray.length;i++){  
				...
				if(fileRef.type =='dir'){
					...
					var f = {"name": fileRef.name, "isParent":false, "_path": fileRef.path, "_name": fileRef.name, "_type": fileRef.type};	
				}
		...
		// 树管理对象
		function GnTreeManagement(){
	  		
		}
	
		GnTreeManagement.prototype.create = function(options){
				var gntree = new Gntree();
				...
				gntree.rightMenu = opt.rightMenu;
				gntree.isUsingFs = opt.isUsingFs;
				gntree.dirOnly = opt.dirOnly;
				gntree.isRightClick = opt.isRightClick;
				if(opt.data.length != 0){
					gntree.data = opt.data;
				}else{
					gntree.data = gntree.transformTozTreeNodes(opt.path);
				}
				gntree.divObj = opt.div;
				gntree.ztreeSetting.edit.enable = opt.editable;
