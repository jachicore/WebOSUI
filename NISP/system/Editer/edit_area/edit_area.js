function EditArea() {
	...
	t.check_line_selection_timer = 50; // the timer delay for modification
										// and/or selection change detection
	 ...
	 * t.textarea="";
	 * 
	 * t.state="declare"; t.code = []; // store highlight syntax for languagues
	 */
	// font datas
	t.lineHeight = 16;
	/*
	 * t.default_font_family= "monospace"; t.default_font_size= 10;
	 */
	...
	// init with settings and ID (area_id is a global var defined by
	// editAreaLoader on iframe creation
	t.id = area_id;
...
EditArea.prototype.init = function() {
	...
	if (syntax_selec = _$("syntax_selection")) {
		...
		for ( var i = 0; i < t.syntax_list.length; i++) {
			...
			option.innerHTML = typeof (dispSyntax) == 'undefined' ? syntax
					.substring(0, 1).toUpperCase()
					+ syntax.substring(1) : dispSyntax;// t.get_translation("syntax_"
														// + syntax, "word");
	...
	// get font size datas
	t
			.set_font(editArea.settings["font_family"],
					editArea.settings["font_size"]);
	...
	/*
	 * if(t.isOpera){ t.editor_area.style.position= "absolute"; }
	 */
	...
	// si le textarea n'est pas grand, un click sous le textarea doit provoquer
	// un focus sur le textarea
	...
	parent.editAreaLoader.add_event(parent.window, "resize",
	...
	parent.editAreaLoader.add_event(window, "unload", function() {
		...
		if (parent.editAreaLoader) {
			parent.editAreaLoader.remove_event(parent.window, "resize",
					editArea.update_size);
			parent.editAreaLoader.remove_event(top.window, "resize",
					editArea.update_size);
		}
	 ...
	 * parent.editAreaLoader.start_time);
	 */
};
...
EditArea.prototype.update_size = function() {
	...
	if (typeof editAreas != 'undefined' && editAreas[editArea.id]
			&& editAreas[editArea.id]["displayed"] == true) {
		if (editArea.fullscreen['isFull']) {
			pd.getElementById("frame_" + editArea.id).style.width = pd
					.getElementsByTagName("html")[0].clientWidth
					+ "px";
			pd.getElementById("frame_" + editArea.id).style.height = pd
					.getElementsByTagName("html")[0].clientHeight
					+ "px";
		}

		if (editArea.tab_browsing_area.style.display == 'block'
			...
			editArea.tab_browsing_area.style.height = (editArea.result.offsetTop
					- editArea.tab_browsing_area.offsetTop - 1)
					+ "px";
		}
...
EditArea.prototype.manage_size = function(onlyOneTime) {
	...
	if (editAreas[this.id]["displayed"] == true && this.textareaFocused) {
		...
		// 3) if there is new lines, we add new line numbers in the line
		// numeration area
...
EditArea.prototype.get_translation = function(word, mode) {
	if (mode == "template")
		return parent.editAreaLoader.translate(word, this.settings["language"],
				mode);
	else
		return parent.editAreaLoader.get_word_translation(word,
...
EditArea.prototype.add_plugin = function(plug_name, plug_obj) {
	for ( var i = 0; i < this.settings["plugins"].length; i++) {
		if (this.settings["plugins"][i] == plug_name) {
			...
			plug_obj.baseURL = parent.editAreaLoader.baseURL + "plugins/"
					+ plug_name + "/";
